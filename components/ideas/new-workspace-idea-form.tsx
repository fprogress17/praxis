...
export function NewWorkspaceIdeaForm({ onCancel }: { onCancel: () => void }) {
  ...
  return (
    <div className="mt-8">
      ...
      <form onSubmit={onSubmit} className="rounded-lg border border-border bg-surface p-6 shadow-soft">
        <p className="mb-5 text-meta leading-6 text-muted">
          Not tied to a channel yet. Saved ideas show up under your channel list in the sidebar.
        </p>

        <div>
          <label htmlFor="ws-idea-body" className="mb-1.5 block text-label font-medium text-foreground">
            Idea
          </label>
          <textarea
            id="ws-idea-body"
            ...
            required
            rows={10}
            placeholder="Angle, name, niche, or notes for a channel you might create…"
